...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        For any position p:
        self.beliefs[p] = Pr(X_t=p | e_{t-1}, e_{t-2}, ..., e_1)
        ...
        self.beliefs[p] = Pr(X_t=p |e_t, e_{t-1}, e_{t-2}, ..., e_1)
        ...
        * When a ghost is captured by Pacman, all beliefs should be
        ...
        if noisyDistance is None:
            ...
            # for every position p, update our beliefs based on the probability prob of observing noisyDistance
            for p in self.legalPositions:
                prob = emissionModel[util.manhattanDistance(p, pacmanPosition)]
                allPossible[p] = prob * self.beliefs[p] if prob > 0 else allPossible[p]
    ...
    def elapseTime(self, gameState):
        ...
        self.beliefs[p] = P(X_t = p | e_{t-1}, e_{t_2} ..., e_1)
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # for every old position p
        for p in self.legalPositions:
            # simulate the probability prob that the ghost moves to newPos and update our old beliefs
            for newPos, prob in self.getPositionDistribution(self.setGhostPosition(gameState, p)).items():
